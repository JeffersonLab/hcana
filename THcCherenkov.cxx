...
Int_t THcCherenkov::ReadDatabase( const TDatime& date )
{
  ...
  fNelem = 1;			// Ahmed. One pair of PMTs for Cherenkov detctor
  ...
  DBRequest list[]={
    ...
    {"cher_neg_gain", fNegGain, kDouble},                 // Ahmed
    {"cher_pos_ped_limit", fPosPedLimit, kInt},           // Ahmed
    {"cher_neg_ped_limit", fNegPedLimit, kInt},           // Ahmed
...
Int_t THcCherenkov::CoarseProcess( TClonesArray&  ) //tracks
{
  ...
  for(Int_t ihit=0; ihit < fNhits; ihit++) {
    ...
    if(!hit) cout << "hit pointer is empyt" << endl;
    ...
    cout << "pos ped = " << fPosPedMean[npmt] << "   raw adc pos = " << fA_Pos[npmt] << endl;
    cout << "neg ped = " << fNegPedMean[npmt] << "   raw adc neg = " << fA_Neg[npmt] << endl;
    cout << endl;

    fA_Pos_p[npmt] = hit->fADC_pos - fPosPedMean[npmt];         // hcer_adc(tube) = hcer_raw_adc(nhit) - hcer_ped(tube)
    fA_Neg_p[npmt] = hit->fADC_neg - fNegPedMean[npmt];         // hcer_adc(tube) = hcer_raw_adc(nhit) - hcer_ped(tube)
    ...
    fNegNpeSum += fNegNpe[npmt];                                // hcer_npe_sum = hcer_npe_sum + hcer_npe(tube)
...
void THcCherenkov::AccumulatePedestals(TClonesArray* rawhits)
{
  ...
  while(ihit < nrawhits) {
    ...
    //    Int_t adcneg = hit->fADC_pos; // this should be hit->fADC_neg Ahmed
