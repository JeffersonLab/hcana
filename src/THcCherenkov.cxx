...
Int_t THcCherenkov::FineProcess( TClonesArray& tracks )
{
  ...
  for (Int_t itrack = 0; itrack < nTracks; itrack++) {
    ...
    if (trackRedChi2Cut && trackBetaCut && trackENormCut && trackDpCut) {
        ...
        fXAtCher = trackXfp + trackTheta * fMirrorZPos;
        fYAtCher = trackYfp + trackPhi   * fMirrorZPos;
        ...
        // cout << "fMirrorZPos = " << fMirrorZPos << "\t" << "fXAtCher = " << fXAtCher << "\t" << "fYAtCher = " << fYAtCher << endl;
        // cout << "=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:=:" << endl;

        for (Int_t iregion = 0; iregion < fNRegions; iregion++) {

            if ((TMath::Abs(fRegionValue[GetIndex(iregion, 0)] - fXAtCher)   < fRegionValue[GetIndex(iregion, 4)]) &&
                (TMath::Abs(fRegionValue[GetIndex(iregion, 1)] - fYAtCher)   < fRegionValue[GetIndex(iregion, 5)]) &&
