...
Int_t THcConfigEvtHandler::Analyze(THaEvData *evdata)
{
  ...
  struct CrateInfo *cinfo = new struct CrateInfo;
...
*/
  ...
  std::map<Int_t, struct CrateInfo *>::iterator it = CrateInfoMap.begin();
  while(it != CrateInfoMap.end()) {
    Int_t roc = it->first;
    struct CrateInfo *cinfo = it->second;
...
Int_t THcConfigEvtHandler::GetNSA(Int_t crate) {
  if(CrateInfoMap.find(crate)!=CrateInfoMap.end()) {
    struct CrateInfo *cinfo = CrateInfoMap[crate];
